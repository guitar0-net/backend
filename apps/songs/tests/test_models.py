...
def test_song_factory_creates_song() -> None:
...
@pytest.mark.django_db
def test_song_with_chords_via_list() -> None:
    chord1 = ChordFactory.create()
    chord2 = ChordFactory.create()

    song = SongFactory.create(chords=[chord1, chord2])
...
@pytest.mark.django_db
def test_song_with_chords_via_count() -> None:
    song = SongFactory.create(chords=3)

    assert song.chords.count() == 3


@pytest.mark.django_db
def test_song_with_schemes_via_list() -> None:
    scheme1 = ImageSchemeFactory.create()
    scheme2 = ImageSchemeFactory.create()

    song = SongFactory.create(schemes=[scheme1, scheme2])
...
@pytest.mark.django_db
def test_song_with_schemes_via_count() -> None:
    song = SongFactory.create(schemes=2)

    assert song.schemes.count() == 2


@pytest.mark.django_db
def test_song_without_chords_and_schemes() -> None:
    song = SongFactory.create()
...
@pytest.mark.django_db
def test_song_title_max_length() -> None:
    long_title = "A" * 200

    song = SongFactory.create(title=long_title)
...
@pytest.mark.django_db
def test_song_get_or_create_by_title() -> None:
    song1 = SongFactory.create(title="Unique Song")
    song2 = SongFactory.create(title="Unique Song")
